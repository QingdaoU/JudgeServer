...
RUN buildDeps='software-properties-common git libtool cmake python-dev python3-pip python-pip libseccomp-dev' && \
    ...
    add-apt-repository ppa:ubuntu-toolchain-r/test && apt-get update && apt-get install -y gcc-8.2 g++-8.2 && \
    ...
    ln -s /usr/bin/gcc-8.2 /usr/bin/gcc && ln -s /usr/bin/g++-8.2 /usr/bin/g++ && \
    mkdir -p /code && \
